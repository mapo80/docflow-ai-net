...
if [ -f "${proj_root}/models/$(basename "${MODEL_PATH}")" ]; then
  curl -s -X POST "${API_URL}/api/v1/jobs" -H "X-API-Key: ${API_KEY}" -H "X-Reasoning: ${REASONING}" -F "file=@${here}/sample.png" | pretty | tee "${here}/process.json" >/dev/null
else
  echo "NOTE: Model not found in ./models — skipping job submission"
fi
