...
function toCode(ruleName: string, rows: Row[]): string {
  const lines: string[] = [];
  ...
  return (
    <Space direction="vertical" size="large" style={{ width:'100%' }}>
        ...
        <Table<Row>
          ...
          footer={() => (
            <Space>
              ...
              <Button onClick={()=> navigator.clipboard.writeText(toCode(name, rows))}>Copy code</Button>
            </Space>
          )}
        />
      </Card>
      ...
      <Card title="Output code (preview)">
        <pre style={{ margin:0, whiteSpace:'pre-wrap' }}>{toCode(name, rows)}</pre>
      </Card>
