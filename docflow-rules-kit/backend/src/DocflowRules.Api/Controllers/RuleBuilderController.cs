...
public class RuleBuilderController : ControllerBase
{
    ...
    [HttpPost("validate")]
    public ActionResult<object> Validate([FromBody] CompileReq req)
    {
        ...
        foreach (var b in req.Blocks)
        {
            ...
            switch (type)
            {
                case "compare":
                    ...
                    if (string.IsNullOrWhiteSpace(op) || !"== != > >= < <=".Split(' ').Contains(op)) errors.Add(new(path+".op", "invalid compare op"));
                    if (string.IsNullOrWhiteSpace(val)) errors.Add(new(path+".value", "value required"));
                    break;
                case "regex":
                    var pat = o["pattern"]?.ToString() ?? "";
                    try { _ = new Regex(pat); } catch { errors.Add(new(path+".pattern", "invalid regex")); }
                    break;
                case "set":
                    var tgt = o["target"]?.ToString();
                    if (string.IsNullOrWhiteSpace(tgt)) errors.Add(new(path+".target", "target is required"));
                    break;
                case "normalize":
                    var kind = o["kind"]?.ToString();
                    if (kind is null || (kind != "number" && kind != "date")) errors.Add(new(path+".kind", "kind must be 'number' or 'date'"));
                    break;
                case "map":
                    var fn = o["fn"]?.ToString();
                    if (string.IsNullOrWhiteSpace(fn)) errors.Add(new(path+".fn", "fn is required (C# expression using 'x')"));
                    break;
                case "deduce":
                    var from = o["from"]?.AsArray()?.Select(x=> x!.ToString()).ToArray() ?? Array.Empty<string>();
                    var outField = o["target"]?.ToString();
                    if (from.Length == 0) errors.Add(new(path+".from", "at least one source field required"));
                    if (string.IsNullOrWhiteSpace(outField)) errors.Add(new(path+".target", "target required"));
                    break;
                case "when":
                    var cond = o["cond"]?.ToString();
                    if (string.IsNullOrWhiteSpace(cond)) errors.Add(new(path+".cond", "condition is required (C# expression)"));
                    break;
                case "calc":
                    var target = o["target"]?.ToString();
                    var expr = o["expr"]?.ToString();
                    if (string.IsNullOrWhiteSpace(target)) errors.Add(new(path+".target", "target required"));
                    if (string.IsNullOrWhiteSpace(expr)) errors.Add(new(path+".expr", "expr required (C# expression, use g.Get<T>("field"))"));
                    break;
            }
            i++;
        }
        return Ok(new { ok = errors.Count == 0, errors });
    }

    [HttpPost("compile")]
    public ActionResult<object> Compile([FromBody] CompileReq req)
    {
        var val = Validate(req) as OkObjectResult;
        var payload = (val!.Value as dynamic)!;
        if (payload.errors.Count > 0) return BadRequest(payload);

        var sb = new StringBuilder();
        sb.AppendLine("// generated by builder");
        sb.AppendLine("void Run(ScriptGlobals g)");
        sb.AppendLine("{");
        ...
        foreach (var b in req.Blocks)
        {
            ...
            else if (type == "compare")
            {
                var op = o["op"]!.ToString();
                var val = o["value"]!.ToString();
                // naive number vs string detection
        ...
        sb.AppendLine("}");
        return Ok(new { ok = true, code = sb.ToString() });
    }
}
