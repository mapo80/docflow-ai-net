using DocflowAi.Net.Application.Abstractions; using DocflowAi.Net.Application.Configuration; using Microsoft.Extensions.Logging; using Microsoft.Extensions.Options; using System.Net.Http; using System.Net.Http.Headers;
...
public sealed class MarkitdownClient : IMarkitdownClient {
    ...
    public async Task<string> ToMarkdownAsync(Stream imageStream, string fileName, CancellationToken ct) {
        ...
        try {
            using var resp = await _http.PostAsync("/markdown", content, ct);
            var payload = await resp.Content.ReadAsStringAsync(ct);
            if (!resp.IsSuccessStatusCode) {
                var detail = payload;
                try { using var err = System.Text.Json.JsonDocument.Parse(payload); detail = err.RootElement.GetProperty("detail").GetString() ?? detail; } catch { }
                _logger.LogError("Markitdown error {Status}: {Detail}", (int)resp.StatusCode, detail);
                throw new MarkitdownException(resp.StatusCode, detail);
            }
            ...
            _logger.LogError(e, "HTTP request to Markitdown failed"); throw new MarkitdownException(System.Net.HttpStatusCode.ServiceUnavailable, e.Message, e);
        }
    }
}
